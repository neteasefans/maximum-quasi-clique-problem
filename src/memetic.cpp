...
//初始化种群
bool oppo_initial_population(solution &sol_cur, solution &sol_best, solution &sol_ms_best, instance ins, int &iter, int tabu_length, int tabu_tenure, int fixed_k,
	int &best_k, double gamma, double &end_time, solution *pop, int pop_size)
{	
	...
	{		
		...
		succ = local_search(sol_cur, sol_best, sol_ms_best, ins, iter, tabu_length, tabu_tenure, fixed_k, best_k, gamma, end_time);
		copy_solution(sol_best, sol_cur, ins);
		if(succ)
			break;
		succ = local_search(sol_cur_oppo, sol_best, sol_ms_best, ins, iter, tabu_length, tabu_tenure, fixed_k, best_k, gamma, end_time);
...
//初始化种群
bool initial_population(solution &sol_cur, solution &sol_best, solution &sol_ms_best, instance ins, int &iter, int tabu_length, int tabu_tenure, int fixed_k,
	int &best_k, double gamma, double &end_time, solution *pop, int pop_size)
{	
	bool succ=false;
	for (int i = 0; i < pop_size; i++)
	{
		...
		succ = local_search(pop[i], sol_best, sol_ms_best, ins, iter, tabu_length, tabu_tenure, fixed_k, best_k, gamma, end_time);
		copy_solution(sol_best, pop[i], ins);
		if(succ)
		   return succ;
		pop[i].random_create(pop[i], ins, fixed_k);
		bool succ = local_search(pop[i], sol_best, sol_ms_best, ins, iter, tabu_length, tabu_tenure, fixed_k, best_k, gamma, end_time);
