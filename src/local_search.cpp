...
bool tabu_search_based_swap(solution &sol_cur, solution &sol_best, instance ins, int &iter_ms, int iter_max, int tabu_length, int tabu_tenure, int necessary_edge, int fixed_k, int *freq)
{
	int tabu_iter = 0;
...
bool tabu_search_constrained_swap(solution &sol_cur, solution &sol_best, instance ins, int &iter_ms, int tabu_length, int tabu_tenure, int necessary_edge, int fixed_k, int *freq)
{
...
bool multi_start(solution &sol_cur, solution &sol_best, solution &sol_ms_best, instance ins, int iter_max, int tabu_length, int tabu_tenure, int fixed_k, int &best_k, double gamma, double &end_time)
{
	int iter_ms = 0;
	...
	while (iter_ms < iter_max)
	{
		...
		bool found = tabu_search_constrained_swap(sol_cur, sol_best, ins, iter_ms, tabu_length, tabu_tenure, necessary_edge, fixed_k, freq);
		if (found == true)
		{
...
bool local_search(solution &sol_cur, solution &sol_best, solution &sol_ms_best, instance ins, int &iter, int tabu_length, int tabu_tenure, int fixed_k, int &best_k, double gamma, double &end_time)
{	
	//int iter_ms = 0;
	int edge_k_clique = fixed_k*(fixed_k - 1) / 2;
	int necessary_edge = ceil(gamma*edge_k_clique);
	...
	if (!feasible)
	{
		found = tabu_search_constrained_swap(sol_cur, sol_best, ins, iter, tabu_length, tabu_tenure, necessary_edge, fixed_k, freq);		
